import {ErrorCodes, GameState, ActionNames, HostActionNames} from '../shared/consts';
import {IBaseEvent} from '../shared/schema';
import {Events} from '../../events';
import {Actions} from '../../actions';
import {Game} from '../game';
import express from 'express';
import expressWs from 'express-ws';
import WebSocket from 'ws';
import {redis} from '../db';
import {Hand, Table, IProfile} from '../schema';
import {routes as profileRoutes} from './profile';
import {routes as gameRoutes} from './game';
import {routes as handRoutes} from './hand';
import {validateToken, getLoggedInUser} from './utils';
export const routes = express.Router() as expressWs.Router;

routes.use('/table', gameRoutes);
routes.use('/hand', handRoutes);
routes.use('/profile', profileRoutes);


async function flushEvents(game: Game): Promise<void> {
  const tableId = game.table.id;
  const events = game.queuedEvents; // push out redis notifications.

  if (events.length > 0) {
    console.log(`[game:${tableId}] Flushing ${events.length} events.`);
    const pipeline = redis.pipeline();
    events.forEach((event) => {
      // alert all hosts listening to this table of the event(s).
      pipeline.publish(tableId, JSON.stringify(event.toJSON()));
      // alert all players at this server.
      broadcast(tableId, event);
    });
    // upload all of these events to redis.
    await pipeline.exec();
  }
}


async function didReceiveMessageForSocket(ws: WebSocket, msg: WebSocket.Data, req: express.Request): Promise<void> {
  // decrypt the event.
  msg = msg.toString();
  if (msg.charAt(0) === '{') {
    const action = Actions.fromString(msg);

    /* validate the event. */
    const user = await getLoggedInUser(req);
    if (!user) {
      return;
    }

    const tableInviteCode = req.query.table;
    if (!tableInviteCode) {
      sendEvent(ws, Events.controlEventForPlayerWithId(user.id).notPermitted('Bad request. Got no invite code.'), user.id);
      ws.close(ErrorCodes.NO_SUCH_TABLE);
      return;
    }
    const table = await Table.findOne({inviteCode: tableInviteCode});
    if (!table) {
      sendEvent(ws, Events.controlEventForPlayerWithId(user.id).notPermitted(`No table with this invite code ${tableInviteCode} available.`), user.id);
      ws.close(ErrorCodes.NO_SUCH_TABLE);
      return;
    }

    // see if the user needs to be a host for this action.
    if (HostActionNames.indexOf(action.type) >= 0) {
      if (user.id !== table.host.valueOf().toString()) {
        sendEvent(ws, Events.controlEventForPlayerWithId(user.id).notPermitted('This action can only be performed by the host.'), user.id);
        return;
      }
    }

    // general table event.
    switch (action.type) {
      case ActionNames.WELCOME:
        console.log(`[wss] ${user.username} said welcome!`);
        const isHost = user.id === table.host.valueOf().toString();
        sendEvent(ws, Events.controlEventForPlayerWithId(user.id).welcome(`Welcome back, ${user.username}!`, isHost), user.id);
        break;
      case ActionNames.REQUEST_START_HAND: {
        console.log(`[wss] ${user.username} would like to start a hand.`);
        const game = await Game.newHand(table);
        await game.begin();
        await game.save();
        await flushEvents(game);
        break;
      }
      case ActionNames.LEAVE: {
        console.log(`[wss] ${user.username} left the table (out of turn fold).`);
        if (table.currentHand) {
          const hand = await Hand.findById(table.currentHand);
          const participant = hand.players.find((p) => p.profile && p.profile.equals(user.id));
          if (participant && participant.inHand) {
            // fold the player.
            participant.folded = true;
            participant.inHand = false;
            hand.markModified('players');
            await hand.save();
          }
        }

        // unseat the player.
        for (let i = 0; i < table.seats.length; i++) {
          if (table.seats[i] && table.seats[i].valueOf().toString() === user.id) {
            table.seats[i] = null;
            break;
          }
        }
        table.markModified('seats');
        await table.save();

        broadcast(table.id, Events.forAll().leftTable(user.id));

        break;
      }
      default: {
        console.log(`[wss] ${user.username} said ${action.type}!`);
        // pass this on to the current hand.
        if (table.currentHand) {
          const hand = await Hand.findById(table.currentHand);
          if (!hand || hand.state === GameState.SHOWDOWN || hand.state === GameState.UNINITIALIZED) {
            sendEvent(ws, Events.controlEventForPlayerWithId(user.id).invalidAuth('An active hand could not be found.'), user.id);
            ws.close(ErrorCodes.NO_SUCH_HAND);
            return;
          }

          // make sure that this player is actually in the hand.
          if (!hand.players.find(player => {
            return player.profile?.valueOf().toString() === user.id
          })) {
            // player not found
            sendEvent(ws, Events.controlEventForPlayerWithId(user.id).notPermitted(`${user.username}: you are not part of this hand.'`), user.id);
            ws.close(ErrorCodes.NOT_PERMITTED);
            return;
          }

          // submit the action to mongodb.
          const game = await Game.fromHandId(hand._id);
          await game.processAction(action);
          await game.save();
          await flushEvents(game);
        }
      }
    }
  }
}

const TABLE_CONNECTION_QUEUE: Record<string, Promise<boolean>> = {};

// what players at each table?
const TABLE_REGISTRY: Record<string, Record<string, WebSocket>> = {};

// what table is each player sitting at?
const PLAYER_REGISTRY: Record<string, string> = {};




export function sendEvent(ws: WebSocket, event: IBaseEvent, userId: string | null) {
  const contents = JSON.stringify(event);
  try{
    ws.send(contents);
  } catch (error) {
    console.error('Error flushing event. Disconnecting user.');
    console.error(error);
    if (userId) {
      unregisterPlayer(userId);
    }
  }
}

export function broadcast(table: string, event: IBaseEvent): void {
  const players = TABLE_REGISTRY[table];
  if (!players) {
    console.log(`No players seated at table ${table} for broadcast.`);
    return;
  }

  const audience = event.audience;
  if (!audience) {
    console.error(`No audience on event ${event.name}`);
    return;
  }

  let targets = [];
  if (audience === 'all') {
    targets = targets.concat(Object.keys(players));
  } else {
    targets = event.audience.split(',');
  }

  for (const target of targets) {
    const ws = players[target];
    sendEvent(ws, event, target);
  }
}

async function beginObservingTable(table: string): Promise<boolean> {
  if (TABLE_REGISTRY[table]) {
    // already observing.
    console.log('[events] Already subscribed to this table.')
    return Promise.resolve(true);
  } else {
    console.log('[events] Setting up subscription..')
  }

  const promise = TABLE_CONNECTION_QUEUE[table];
  if (promise) {
    console.log('[events] Subscription was already in flight.')
    // request in flight.
    return await TABLE_CONNECTION_QUEUE[table];
  }

  TABLE_REGISTRY[table] = {};
  const tableId = 'table:' + table;

  console.log(`[events] Server subscribing to table: ${table}`);
  const p = new Promise<boolean>(async (resolve, reject) => {
    redis.subscribe(tableId, (err: Error, count: number) => {
      // null-out in flight request.
      TABLE_CONNECTION_QUEUE[table] = null;

      if (err) {
          console.error(`[events] Server failed to subscribe to ${tableId} (total subscriptions: ${count})`);
          console.error(err);
          resolve(false);
          return false;
      }

      console.log(`[events] Server successfully subscribed to ${tableId} (total subscriptions: ${count})`);
      resolve(true);
      return true;
    });
  });

  TABLE_CONNECTION_QUEUE[table] = p;
  return await p;
}

async function registerPlayer(ws: WebSocket, id: string, tableId: string): Promise<void> {
  if (PLAYER_REGISTRY[id]) {
    // TODO(correctness): Should we just unregister here?
    console.error(`Player was already seated at table ${PLAYER_REGISTRY[id]}`);
    unregisterPlayer(id);
  }

  PLAYER_REGISTRY[id] = tableId;

  const tableObj = TABLE_REGISTRY[tableId];
  if (!tableObj) {
    // this table isn't being watched on this server. create an entry.
    const success = await beginObservingTable(tableId);
    if (!success) {
      throw new Error(`Failed to observe table ${tableId}.`);
    }
  } else {
    console.log(`[events] Already subscribed to ${tableId}`);
  }

  // seat the player.
  TABLE_REGISTRY[tableId][id] = ws;
  console.log(`Player ${id} was registered at table ${tableId}`);
}

function redisKeyForTable(table: string) {
  return 'table:' + table;
}

async function unregisterPlayer(user: string): Promise<void> {
  const tableId = PLAYER_REGISTRY[user];
  if (!tableId) {
    // wasn't already seated.
    console.error(`[player] ${user} was not seated at a table. Nothing to do.`);
    return;
  }

  // remove the entry.
  delete PLAYER_REGISTRY[user];

  const tableObj = TABLE_REGISTRY[tableId];
  if (tableObj[user]) {
    console.log(`Unseated user with id ${user} from table ${tableId}`);
    delete tableObj[user];
  }

  // clean up the table.
  if (Object.keys(tableObj).length === 0) {
    // can unsubscribe.
    const tid = redisKeyForTable(tableId);
    await redis.unsubscribe(tid);
    console.log(`Cleaned up subscription to ${tid}`);
  }
}

redis.on("message", (channel: string, message: string) => {
  if (channel.startsWith('table:')) {
    // this is a table event broadcast. send to all clients.
    const tid = channel.slice(6);
    const event = Events.fromString(message);

    broadcast(tid, event);
    console.log(`Table[${tid}]: Re-broadcasted event ${event.name} to ${event.audience} participants.`);
  }
});


async function onReceiveNewPlayer(ws: WebSocket, r: express.Request): Promise<IProfile> {
  // opened a new connection.
  const user  = r.query.user;
  const token = r.query.token;
  const table = r.query.table;

  console.log(`[wss] Got new request to seat user (id): ${user} at table ${table}`);

  // valid user token.
  const valid = await validateToken(user, token, ws);
  if (!valid) {
    sendEvent(ws, Events.controlEventForPlayerWithId(user).invalidAuth(`Incorrect token received ${token}.`), null);
    ws.close(ErrorCodes.NOT_PERMITTED);
    return;
  }

  // seat the user at the table.
  const mTable = await Table.findOne({inviteCode: table})

  console.log(`[wss] Seating ${valid.username}`);
  // see if you're sitting already.
  if (mTable.seats && mTable.seats.length === 8) {
    let isSeated = !!mTable.seats.find((playerId) => {
      return playerId !== null && (playerId.valueOf().toString() === valid.id);
    });
    if (!isSeated) {
      // add the player into this seat chart.
      for (let i = 0; i < mTable.seats.length; i++) {
        if (!mTable.seats[i]) {
          mTable.seats[i] = valid.id;
          console.log(`Seated player ${valid.username} at seat ${i+1}`);
          isSeated = true;
          break;
        }
      }

      if (!isSeated) {
        console.error('[seating] No seats available for this player.');
        sendEvent(ws, Events.controlEventForPlayerWithId(user).notPermitted(`No room left at table.`), null);
        ws.close(ErrorCodes.NOT_PERMITTED);
        return;
      }
    } else {
      console.error(`${valid.username} was already seated.`);
    }
  } else {
    mTable.seats = [null, null, null, null, null, null, null, null];
    mTable.seats[0] = valid.id;
    console.log(`Created new seats + put player ${valid.username} at seat 1`);
  }

  console.log(`Updated Table seats: ${JSON.stringify(mTable.seats)}`);
  mTable.markModified('seats'); // because it's an array, we have to say that it's been updated.
  await mTable.save();
  console.log('Saved!');

  // register this connection.
  await registerPlayer(ws, valid.id, mTable.id);

  return valid;
}


/*
 * Streaming endpoint.
 */
routes.ws('/stream', async (wss: WebSocket, req: express.Request) => {
  console.log('[wss] Got stream request.');
  const user = await onReceiveNewPlayer(wss, req);
  wss.on('message', async (msg) => {
    didReceiveMessageForSocket(wss, msg, req);
  });
  wss.on('close', async (ws: WebSocket, _req: express.Request) => {
    // unsubscribe this user.
    console.log('[wss] conn closed');
    await unregisterPlayer(user.id);
  });

  wss.on('error', async (ws: WebSocket, err: Error) => {
    console.log('[wss] conn error: ');
    console.error(err);
  });
});
